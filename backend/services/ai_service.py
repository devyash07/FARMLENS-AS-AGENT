...
def _torch_predict(image_bytes: bytes) -> dict:
    ...
    try:
        ...
        # Generate Grad-CAM explainability heatmap for infected plants
        gradcam_b64 = ""
        if not is_healthy:
            try:
                from utils.gradcam import get_pytorch_gradcam_b64
                print("[AI] Generating Grad-CAM explainability heatmap for MobileNetV2...")
                gradcam_b64 = get_pytorch_gradcam_b64(model, image_bytes, x, index)
            except Exception as ge:
                print(f"[AI] Grad-CAM generation failed: {ge}")
...
def predict(image_bytes: Optional[bytes] = None, language: str = "en") -> dict:
    ...
    if not heatmap_b64 and result["status"] == "Infected":
